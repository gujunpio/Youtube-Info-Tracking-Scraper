 ...
 * Also handles URL-encoded Unicode handles (e.g. @%EC%9D%80%EA%B0%80...)
 */
function isValidYoutubeChannel(url) {
  // Try decoding first so encoded Unicode handles become readable
  let testUrl = url;
  try { testUrl = decodeURIComponent(url); } catch { /* keep original */ }
  // Match both decoded Unicode characters and percent-encoded sequences
  return /^https?:\/\/(www\.)?youtube\.com\/((@[^\s\/]+)|(channel\/[\w-]+)|(c\/[\w.-]+)|(user\/[\w.-]+))/i.test(testUrl)
      || /^https?:\/\/(www\.)?youtube\.com\/((@[^\s\/]+)|(channel\/[\w-]+)|(c\/[\w.-]+)|(user\/[\w.-]+))/i.test(url);
}
