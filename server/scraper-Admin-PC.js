...
async function scrapeChannel(channelUrl, seenIds = new Set()) {
  ...
  try {
    ...
    // --- Language Detection (CLD3 — same engine as Chrome) ---
    // Use video titles as PRIMARY signal (never auto-translated by YouTube)
    const titles = data.videos.map(v => v.title).filter(Boolean);
    let lang;

    const textsForDetection = [...titles];
    // Add channel description as a single secondary signal (may be translated)
    if (data.channelDescription && data.channelDescription.length > 10) {
      textsForDetection.push(data.channelDescription);
      console.log('[Scraper] Channel desc (secondary):', data.channelDescription.substring(0, 80));
    }

    lang = await detectLanguage(textsForDetection);
